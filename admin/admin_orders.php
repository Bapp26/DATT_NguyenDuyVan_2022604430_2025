...
if (isset($_POST['update_status'])) {
    ...
    if ($new_status !== "") {
        ...
        if ($new_status == 'Đã hủy') {
            mysqli_query($conn, "UPDATE football_fields SET status = 'Đang hoạt động' WHERE id = '$field_id'");
        }
...
if (isset($_POST['cancel_recurring'])) {
    ...
    if (isset($_FILES['refund_image']) && $_FILES['refund_image']['error'] === 0) {
        ...
        if (move_uploaded_file($image['tmp_name'], $target_path)) {
            // Một số cột như cancel_reason, refund_image, cancel_date có thể không tồn tại trong bảng recurring_bookings
            // Cập nhật tối thiểu trạng thái để tránh lỗi cột không tồn tại
            $update_query = mysqli_query($conn, "UPDATE recurring_bookings SET status = 'Đã hủy' WHERE id = '$recurring_id'");
