            ...
            <div class="col-lg-8">
                <div class="booking-form-card">
                    ...
                    <form method="POST" id="bookingForm">
                        ...
                        <button type="button" class="btn btn-booking mt-4" onclick="showPaymentModal()" style="font-weight: bold; color: white; background: #36be36;" >
                            Đặt sân
                        </button>
                    </form>
                </div>
...
//     if (!mainForm.checkValidity() || !validateBooking()) {
//         mainForm.reportValidity(); // hiện thông báo required mặc định
//         return;
//     }
...
//     .then(data => {
//         if (data.isBooked) {
//       alert(`Sân ${data.fieldName} đã được đặt từ ${data.startTime} đến ${data.endTime} ngày ${data.bookingDate}`); // hiện thông báo sân đã đặt
//         } else {
//             // Copy dữ liệu sang modal
//             document.getElementById('modal_field_id').value = formData.get('field_id');
//             document.getElementById('modal_booking_date').value = formData.get('booking_date');
//             document.getElementById('modal_start_time').value = formData.get('start_time');
//             document.getElementById('modal_duration').value = formData.get('duration');
//             document.getElementById('modal_rent_ball').value = mainForm.querySelector('[name="rent_ball"]').checked ? 1 : 0;
//             document.getElementById('modal_rent_uniform').value = mainForm.querySelector('[name="rent_uniform"]').checked ? 1 : 0;
//             document.getElementById('modal_note').value = formData.get('note');
//             document.getElementById('modal_payment_method').value = formData.get('payment_method');

//             // Hiển thị phương thức thanh toán tương ứng
//             showPaymentMethod(formData.get('payment_method'));
...
//     })
//     .catch(err => {
//         alert("Có lỗi khi kiểm tra lịch đặt sân!");
//         console.error(err);
//     });
// }
...
function showPaymentModal() {
    ...
    errorBox.classList.add("d-none"); // ẩn lỗi cũ
    ...
    .then(data => {
        if (data.isBooked) {
            ...
            errorBox.innerHTML = `${data.fieldName} đã được đặt trong khung giờ ${data.startTime} - ${data.endTime} ngày ${new Date(data.bookingDate).toLocaleDateString('vi-VN')}. Vui lòng chọn khung giờ khác!`;
            errorBox.classList.remove("d-none");

            // Cuộn lên đầu form để thấy lỗi
            errorBox.scrollIntoView({ behavior: "smooth", block: "start" });
        } else {
            // Không trùng → copy dữ liệu sang modal
            document.getElementById('modal_field_id').value = formData.get('field_id');
            document.getElementById('modal_booking_date').value = formData.get('booking_date');
            document.getElementById('modal_start_time').value = formData.get('start_time');
            document.getElementById('modal_duration').value = formData.get('duration');
            ...
            // Hiển thị phương thức thanh toán tương ứng (QR, MoMo, VNPay)
            showPaymentMethod(formData.get('payment_method'));
    ...
    .catch(err => {
        console.error(err);
        alert("Có lỗi xảy ra khi kiểm tra đặt sân!");
    });
}
